...
function buildResultsEmail(result: AnalysisResult): string {
  ...
  <div style="max-width:560px;margin:32px auto;background:white;border-radius:16px;border:1px solid #e2e8f0;overflow:hidden">
    ...
    <div style="padding:24px 32px">
      ...
      <div style="margin-top:20px;text-align:center">
        <a href="https://diagnozisom.hu" style="display:inline-block;background:#0ea5e9;color:white;padding:10px 24px;border-radius:20px;font-size:13px;font-weight:600;text-decoration:none">
          Új elemzés indítása
        </a>
      </div>
    </div>
    ...
    <div style="padding:16px 32px;background:#f8fafc;border-top:1px solid #f1f5f9;text-align:center">
      <p style="margin:0;font-size:11px;color:#94a3b8">
        © ${new Date().getFullYear()} Diagnózisom.hu · 
        <a href="https://diagnozisom.hu/adatvedelem" style="color:#94a3b8">Adatvédelem</a>
      </p>
    </div>
  </div>
</body></html>`
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { email, result, wants_contact, name, phone } = body as {
      email: string
    ...
    if (!resendKey) {
      console.warn('[SendResults] RESEND_API_KEY not set')
    ...
    // 2. If user wants contact, save as lead in Supabase
    if (wants_contact) {
      try {
        ...
        if (url && key) {
          await fetch(`${url}/rest/v1/leads`, {
            ...
            body: JSON.stringify({
              ...
              email,
              phone: phone || null,
              professional_followup: true,
            }),
          })
        }
      } catch (e) {
        console.error('[SendResults] Supabase lead save error:', e)
        // Don't fail the whole request — email was sent, lead save is secondary
      }
    }
